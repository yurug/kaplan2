...
static void arena_grow(size_t need) {
    ...
    g_arena = c;
    /* H1 (madvise variant): drop pages from chunks older than 4 from RSS.
       Pages stay mapped — kernel re-faults on access. */
    kt_arena_chunk* p = c->prev;
    int keep = 4;
    while (p && keep > 0) { p = p->prev; keep--; }
    while (p) {
        size_t mapped = (size_t)(p->end - (char*)p);
        size_t pgsz = 4096;
        uintptr_t addr = ((uintptr_t)p + pgsz - 1) & ~(uintptr_t)(pgsz - 1);
        size_t off = addr - (uintptr_t)p;
        if (mapped > off) madvise((void*)addr, (mapped - off) & ~(pgsz - 1), MADV_DONTNEED);
        p = p->prev;
    }
}
