...
static __thread kt_arena_chunk* g_arena_ring[4] = {NULL,NULL,NULL,NULL};
static __thread int g_arena_ring_pos = 0;

static void arena_grow(size_t need) {
    size_t chunk_payload = ARENA_CHUNK_SIZE;
    if (need > chunk_payload) chunk_payload = need;
    /* Ring-buffer arena: H1 hypothesis test only.  Cycles 4 chunks; when
       all are full, oldest is reset and reused.  UNSAFE: existing pointers
       into reset chunk are dangling.  Probes the cache-locality effect. */
    kt_arena_chunk* c;
    int slot = g_arena_ring_pos;
    if (g_arena_ring[slot] != NULL && g_arena_ring[slot]->end - g_arena_ring[slot]->data >= (long)chunk_payload) {
        c = g_arena_ring[slot];
        c->bump = c->data;
    } else {
        c = (kt_arena_chunk*)malloc(sizeof(*c) + chunk_payload);
        c->bump = c->data;
