...
| 4b cont. — adopt6 cascade redesign | ✅ **STRUCTURAL BLOCKER RESOLVED + ADOPT6 FOUNDATION + FULL DISPATCHER + §12.4 END-TO-END**: new `Cadeque6/Adopt6.v` (~7100 lines, ~210 theorems) introduces `CadCellA6` with adopt6 pointer, embed/extract round-trip, full 5-op imperative DSL with WC O(1) for ALL ops including pop/eject *cascade* (previously O(depth)).  Foundation: `heap_represents_cad_a6` inductive relation + persistence-under-alloc + determinism + `adopt6_wf_at` shape predicate.  Coverage matrix on the rich type: WC bounds (5 ops), full cost-exact matrix, sequence-correctness + list-refinement + input-persistence (push/inject 3 + pop/eject 6 shallow + 2 GENERALIZED CSingle + concat 4 simple + 2 empty + dispatcher SS/DD), **18 FLAGSHIP FULL CONTRACT bundles**, **12 adopt6 wf-at-result theorems**, **`cad_concat_imp_a6_full` dispatcher** wiring all 4 SS/DS/SD/DD paths (WC ≤ 8) + 6 path-equivalence theorems.  **§12.4 END-TO-END**: new `Cadeque6/RepairS12.v` (8 abstract repair-case functions + seq laws) + 8 operational repair cases on Adopt6 (cost ≤ 2 each) + 3 refinement bridges + case-selection logic + unified `repair_replace_imp_a6` dispatcher + **8 composed pop/eject + repair operations** with WC ≤ 6 (Phase 1 + Phase 2 + Phase 3 in one pipeline). | `72096a3`–`e895d02` (~57 commits) |
