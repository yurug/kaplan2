...
| 4b cont. — adopt6 cascade redesign | ✅ **STRUCTURAL BLOCKER RESOLVED + ADOPT6 FOUNDATION + FULL DISPATCHER**: new `Cadeque6/Adopt6.v` (6150+ lines, 185 theorems) introduces `CadCellA6` with adopt6 pointer, embed/extract round-trip, full 5-op imperative DSL with WC O(1) for ALL ops including pop/eject *cascade* (previously O(depth)).  Foundation: `heap_represents_cad_a6` inductive relation + persistence-under-alloc + determinism + `adopt6_wf_at` shape predicate.  Coverage matrix on the rich type: WC bounds (5 ops), full cost-exact matrix (every shallow shape costs ≤ 4), sequence-correctness (push/inject 3 + pop/eject 6 shallow + 2 GENERALIZED CSingle with ANY child + concat 4 simple + 2 empty + dispatcher SS/DD via equiv), list-level refinement (push/inject 3 + pop/eject 6 + 2 gen + concat 4 simple + 2 empty + dispatcher SS/DD), input-persistence (every op + every shape), **18 FLAGSHIP FULL CONTRACT bundles** + 6 pop/eject 3-piece variants + **12 adopt6 wf-at-result theorems** + **`cad_concat_imp_a6_full` dispatcher** wiring all 4 SS/DS/SD/DD paths (WC ≤ 8) + 6 path-equivalence theorems.  Concrete vm_compute-verified end-to-end examples. | `72096a3`–`eae2f95` (~50 commits) |
