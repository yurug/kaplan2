...
| 4b cont. — adopt6 cascade redesign | ✅ **§12.4 USER-FACING API FULLY CLOSED + GLOBAL ADOPT6 INVARIANT PRESERVED**: `Cadeque6/Adopt6.v` (9256 lines, 336 theorems) with CadCellA6 + adopt6 pointer + embed/extract + full 5-op imperative DSL WC O(1) including cascade.  Foundation: `heap_represents_cad_a6` + persistence + determinism + `adopt6_wf_at`.  Coverage: WC bounds, cost-exact matrix, seq + list-refinement + input-persistence (push/inject 3 + pop/eject 6 shallow + 2 generalized CSingle + concat 4 simple + 2 empty + dispatcher SS/DD), **18 FLAGSHIP FULL CONTRACT bundles**, **20 adopt6 wf-at-result theorems**, **`cad_concat_imp_a6_full` dispatcher** (WC ≤ 8).  **§12.4 END-TO-END**: `RepairS12.v` + 8 operational §12.4 cases (cost ≤ 2 + full contracts) + `repair_replace_imp_a6` + 8 composed pop/eject+repair (WC ≤ 6) + 8 termination wrappers.  **STORED-READ**: read_stored_small/big_imp_a6 (cost = 1) + 2 full Case 1b pipelines (WC ≤ 3).  **HEADLINE SINGLE-CALL API — ALL 7 PATHS**: pop 1b-left, 1a-left, 2a-only, 2c-empty (WC ≤ 5), 2c-twosided (WC ≤ 6); eject 1b-right, 2b-only (WC ≤ 5).  Each with cost + termination + seq + list_correct theorems.  **GLOBAL ADOPT6 INVARIANT PRESERVATION**: `adopt6_globally_wf` predicate + 4 building-block alloc lemmas + 8 §12.4 case preservation theorems (one per repair case).  Enables safe consecutive operations. | `72096a3`–`3f38705` (~70 commits) |
