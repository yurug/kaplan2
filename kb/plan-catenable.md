...
| 4b cont. — adopt6 cascade redesign | ✅ **STRUCTURAL BLOCKER RESOLVED**: new `Cadeque6/Adopt6.v` (1500+ lines, 51 theorems) introduces `CadCellA6` with adopt6 pointer on cadeque cells, embed/extract round-trip, full 5-op imperative DSL with WC O(1) for ALL ops including pop/eject *cascade* (previously O(depth)).  Cost bounds proven over ANY heap and ANY input: push/inject/pop/eject ≤ 4, concat ≤ 8.  Plus `heap_represents_cad_a6` inductive relation, persistence-under-alloc lemmas, sequence-correctness for push/inject (3 input shapes each) and pop shallow case. | `72096a3`–`0d66d29` (~12 commits) |
