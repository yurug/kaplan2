...
| 4b cont. — adopt6 cascade redesign | ✅ **STRUCTURAL BLOCKER RESOLVED + ADOPT6 FOUNDATION + FULL DISPATCHER + §12.4 END-TO-END + FULL CONTRACT MATRIX + STORED-READ PRIMITIVES**: `Cadeque6/Adopt6.v` (~7700 lines, 281 theorems) introduces `CadCellA6` with adopt6 pointer, embed/extract round-trip, full 5-op imperative DSL with WC O(1) for ALL ops including pop/eject *cascade* (previously O(depth)).  Foundation: `heap_represents_cad_a6` + persistence + determinism + `adopt6_wf_at`.  Coverage: WC bounds (5 ops), full cost-exact matrix, sequence-correctness + list-refinement + input-persistence (push/inject 3 + pop/eject 6 shallow + 2 GENERALIZED CSingle + concat 4 simple + 2 empty + dispatcher SS/DD), **18 FLAGSHIP FULL CONTRACT bundles**, **20 adopt6 wf-at-result theorems**, **`cad_concat_imp_a6_full` dispatcher** (WC ≤ 8).  **§12.4 END-TO-END**: `Cadeque6/RepairS12.v` (8 abstract + seq laws) + 8 operational §12.4 cases (cost ≤ 2 each with full contracts) + `repair_replace_imp_a6` dispatcher + **8 composed pop/eject + repair operations** (WC ≤ 6) + 8 termination wrappers.  **STORED-READ PRIMITIVES**: `read_stored_small_imp_a6` / `read_stored_big_imp_a6` (cost = 1 each) + **2 full Case 1b pipelines** (read stored + repair, WC ≤ 3, self-contained). | `72096a3`–`f1b5113` (~62 commits) |
