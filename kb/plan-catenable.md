...
| 4b cont. — adopt6 cascade redesign | ✅ **STRUCTURAL BLOCKER RESOLVED + FULL COVERAGE**: new `Cadeque6/Adopt6.v` (3750+ lines, 113 theorems) introduces `CadCellA6` with adopt6 pointer, embed/extract round-trip, full 5-op imperative DSL with WC O(1) for ALL ops including pop/eject *cascade* (previously O(depth)).  Foundation: `heap_represents_cad_a6` inductive relation + persistence-under-alloc + determinism.  Coverage matrix on the rich type: WC bounds (5 ops), cost-exact (per shape), sequence-correctness (push/inject 3 shapes + pop/eject shallow cases + concat 4 simple sub-ops), list-level refinement (push/inject 3 shapes + concat 4 simple), input-persistence (push/inject + concat), **10 FLAGSHIP FULL CONTRACT bundles**. | `72096a3`–`638f410` (~30 commits) |
