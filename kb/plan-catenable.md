...
| 4b cont. — adopt6 cascade redesign | ✅ **§12.4 FULLY CLOSED — API + GLOBAL ADOPT6 INVARIANT + GREEN4 RESULT REGULARITY**: `Cadeque6/Adopt6.v` (9256 lines, 336 theorems) + `Cadeque6/RepairS12.v` (497 lines, 31 theorems).  CadCellA6 + adopt6 pointer + embed/extract + full 5-op imperative DSL WC O(1) including cascade.  Foundation: `heap_represents_cad_a6` + persistence + determinism + `adopt6_wf_at`.  Coverage: WC bounds, cost-exact matrix, seq + list-refinement + input-persistence + **18 FLAGSHIP FULL CONTRACT bundles** + **20 adopt6 wf-at-result** + **`cad_concat_imp_a6_full`** (WC ≤ 8).  **§12.4 END-TO-END**: 8 operational repair cases (cost ≤ 2 + full contracts) + `repair_replace_imp_a6` + 8 composed pop/eject+repair (WC ≤ 6) + 8 termination wrappers.  **HEADLINE SINGLE-CALL API — ALL 7 PATHS**: pop 1b-left/1a-left/2a-only/2c-empty (WC ≤ 5)/2c-twosided (WC ≤ 6); eject 1b-right/2b-only (WC ≤ 5).  Each with cost + termination + seq + list_correct.  **GLOBAL ADOPT6 INVARIANT**: `adopt6_globally_wf` + 4 alloc lemmas + 8 §12.4 case preservation theorems.  **REGULARITY (Green4 RESULT)**: 8 per-case `repair_case_NN_green` theorems witnessing `triple_color = Green4` under buffer-size hypotheses. | `72096a3`–`f94387e` (~71 commits) |
