...
id: plan-catenable
domain: planning
related: [spec/why-bounded-cascade, architecture/overview, architecture/decisions/adr-0001-public-api-encoding, spec/kcadeque-design]
status: superseded
...
| 4b cont. — adopt6 cascade redesign | ✅ **§12.4 FULLY CLOSED — 10 HEADLINE OPS × 7-PROPERTY MATRIX**: `Cadeque6/Adopt6.v` (10500+ lines) + `Cadeque6/RepairS12.v` (497 lines).  CadCellA6 + adopt6 pointer + embed/extract + full 5-op imperative DSL WC O(1) including cascade.  Foundation: `heap_represents_cad_a6` + persistence + determinism + `adopt6_wf_at`.  Coverage: WC bounds, cost-exact matrix, seq + list-refinement + input-persistence + **18 FLAGSHIP FULL CONTRACT bundles (basic ops)** + **20 adopt6 wf-at-result** + **`cad_concat_imp_a6_full`** (WC ≤ 8).  **§12.4 HEADLINE API — ALL 10 PATHS**: pop {1b-left, 1a-left, 2a-only, 2c-empty, 2c-twosided}; eject {1b-right, 2b-only, 2a-only, 2c-empty, 2c-twosided} — each with WC ≤ 5/6 cost + termination + seq + list_correct + adopt6_globally_wf + heap_top_green + bundled FLAGSHIP contract (10 × 7 = 70 per-op theorems).  **GLOBAL ADOPT6 INVARIANT**: `adopt6_globally_wf` + 4 alloc lemmas + 10 per-headline-op preservation theorems.  **BRIDGE TO ABSTRACT REGULARITY**: `heap_top_green H l` predicate + 10 `_top_green` theorems witnessing the Green4 top-color (manual §10.6) restored by §12.4 repair. | `72096a3`–`90d46c3` (~75 commits) |
