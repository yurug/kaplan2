...
| 4b cont. — adopt6 cascade redesign | ✅ **§12.4 USER-FACING API CLOSED**: `Cadeque6/Adopt6.v` (~7850 lines, 289 theorems) with CadCellA6 + adopt6 pointer + embed/extract round-trip + full 5-op imperative DSL WC O(1) including cascade.  Foundation: `heap_represents_cad_a6` + persistence + determinism + `adopt6_wf_at`.  Coverage: WC bounds, cost-exact matrix, seq + list-refinement + input-persistence (push/inject 3 + pop/eject 6 shallow + 2 generalized CSingle + concat 4 simple + 2 empty + dispatcher SS/DD), **18 FLAGSHIP FULL CONTRACT bundles**, **20 adopt6 wf-at-result theorems**, **`cad_concat_imp_a6_full` dispatcher** (WC ≤ 8).  **§12.4 END-TO-END**: `RepairS12.v` (8 abstract + seq laws) + 8 operational repair cases (cost ≤ 2 + full contracts) + `repair_replace_imp_a6` + **8 composed pop/eject+repair** (WC ≤ 6) + 8 termination wrappers.  **STORED-READ**: read_stored_small/big_imp_a6 (cost = 1) + 2 full Case 1b pipelines (WC ≤ 3).  **HEADLINE SINGLE-CALL API**: `cad_pop_full_repair_1b_left_imp_a6 (lA ls lc')` and `cad_eject_full_repair_1b_right_imp_a6 (lA lc' ls)` — fully self-contained WC ≤ 5 (3 reads + 2 allocs) with termination wrappers. | `72096a3`–`4fd48e4` (~63 commits) |
