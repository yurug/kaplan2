...
| 4b cont. — adopt6 cascade redesign | ✅ **STRUCTURAL BLOCKER RESOLVED + FULL COVERAGE**: new `Cadeque6/Adopt6.v` (5150+ lines, 145 theorems) introduces `CadCellA6` with adopt6 pointer, embed/extract round-trip, full 5-op imperative DSL with WC O(1) for ALL ops including pop/eject *cascade* (previously O(depth)).  Foundation: `heap_represents_cad_a6` inductive relation + persistence-under-alloc + determinism.  Coverage matrix on the rich type: WC bounds (5 ops), full cost-exact matrix (every shallow shape costs ≤ 4), sequence-correctness (push/inject 3 shapes + pop/eject 6 shallow shapes + 2 GENERALIZED CSingle pop/eject with ANY middle child + concat 4 simple sub-ops), list-level refinement (push/inject 3 shapes + pop/eject 6 shallow + 2 generalized + concat 4 simple), input-persistence (push/inject + pop/eject 6 shallow + empty + concat), **18 FLAGSHIP FULL CONTRACT bundles** (4 concat simple + 3 push + 3 inject + 6 pop/eject + 2 generalized CSingle pop/eject) + 6 pop/eject 3-piece variants. | `72096a3`–`b1d671f` (~40 commits) |
