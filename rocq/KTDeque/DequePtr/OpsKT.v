...
(* green-headed chain.                                                        *)
(*                                                                            *)
(* The "O(1) repair" primitive that ensure_green calls.  Three structural    *)
(* cases, mirroring Viennot.  No recursion — by the regularity invariant     *)
(* the level below the Red top is Green or absent, so the dispatch makes     *)
(* exactly one rebalancing decision.                                         *)
(*                                                                            *)
(*   Case 1 — depth-1 Red over Ending:                                       *)
(*     Bottom of the chain.  Collapse the three buffers (pre, ending, suf)  *)
(*     into a fresh Green chain via [make_small].                            *)
(*                                                                            *)
(*   Case 2 — depth-1 Red over a depth-1+ ChainCons:                         *)
(*     Tail is Green by regularity.  Merge our outer pre/suf into tail's    *)
(*     outer pre/suf via green_prefix_concat / green_suffix_concat; the     *)
(*     result is a single Green link replacing both the old top and tail.   *)
(*                                                                            *)
(*   Case 3 — depth-≥2 Red:                                                  *)
(*     Split the yellow run at the outer level: the outermost level becomes *)
(*     a Green link, the rest of the run becomes a (tagged-Red) sub-packet *)
(*     that prepends to the tail.                                            *)
(*                                                                            *)
(* The cost is bounded by a structural constant (NF_PUSH_PKT_FULL = 9 in   *)
(* CostMonad-instrumented form; see DequePtr/Footprint.v).                  *)
(* ========================================================================== *)
...
(*                                                                            *)
(* This is the WC-O(1) repair primitive (see kb/spec/why-bounded-cascade.md   *)
(* §2 for why).  The shape is intentionally trivial:                          *)
(*                                                                            *)
(*   * if the chain is empty (Ending), no work — the buffer's already a      *)
(*     valid one-cell chain.                                                  *)
(*   * if the top packet is Green or Yellow, no work — those colors are     *)
(*     stable.                                                                *)
(*   * if the top packet is Red, fire green_of_red exactly once.  Because    *)
(*     by regularity the level below the Red top is Green or absent,        *)
(*     green_of_red itself is non-recursive (3 structural cases, all O(1)). *)
(*                                                                            *)
(* So: each public op (push/inject/pop/eject) does at most one buffer       *)
(* update + at most one ensure_green call + at most one green_of_red — all  *)
(* of which are O(1).  That's the entire WC O(1) bound.                      *)
(*                                                                            *)
(* The pkt_outer_color helper below derives a packet's color from its       *)
(* buffer sizes; the chain link's authoritative color is the explicit tag   *)
(* carried in [KCons], not the structurally-derived one (see file header   *)
(* for why the tag is contextual).                                            *)
(* ========================================================================== *)
