...
Lemma push_kt4_preserves_colors_leveled :
  forall A (x : E.t A) (c c' : KChain A),
    ...
    push_kt4 x c = PushOk c' -> colors_consistent c' /\ well_leveled c'.
Proof.
  intros A x c c' HI Hx.
  destruct HI as [Hreg [Hcc Hwl]].
  ...
  - (* KEnding *)
    ...
    (* B5 split leaves the packet_levels goal *)
  ...
  - pose proof Hwl as Hwl0. cbn in Hwl0. destruct Hwl0 as [Hpkt Hwltail].
    ...
    + (* Green: wrap *)
      ...
      destruct pre as [|a|a b1|a b1 c1|a b1 c1 d|];
        cbn in Hgp; try contradiction;
        cbn -[yellow_wrap_pr]; intros Hp;
        cbn in Hpre;
        repeat match goal with H : _ /\ _ |- _ => destruct H end.
      * (* B2 -> wrap with B3 *)
        eapply yellow_wrap_pr_preserves_colors_leveled
          with (k := 0) (pre := B3 x a b1);
         [ cbn; exact I
         | apply buf5_green_shape_not_red; exact Hgs
         | exact Hyr
         | exact Htc
         | exact Htail
         | constructor; [cbn; repeat split; auto | exact Hinner | exact Hsuf]
         | exact Hwltail
         | exact Hp ].
      * (* B3 -> wrap with B4 *)
        eapply yellow_wrap_pr_preserves_colors_leveled
          with (k := 0) (pre := B4 x a b1 c1);
         [ cbn; exact I
         | apply buf5_green_shape_not_red; exact Hgs
         | exact Hyr
         | exact Htc
    ...
    + (* Yellow *)
      ...
      destruct pre as [|a|a b1|a b1 c1|a b1 c1 d|];
        cbn in Hnp; try contradiction;
        cbn -[green_of_red_k]; intros Hp; cbn in Hpre;
        repeat match goal with H : _ /\ _ |- _ => destruct H end.
      * (* B1 -> B2 *)
      ...
      * (* B4: overflow repair *)
        destruct (green_of_red_k
                    (KCons Red (PNode (B5 x a b1 c1 d) i suf) tail))
        ...
        { cbn. split; [| exact Hwltail].
          eapply packet_levels_push_overflow_pre; [exact Hx |].
          constructor; [cbn; repeat split; auto | exact Hinner | exact Hsuf]. }
        split.
...
Lemma push_kt4_preserves_I_kt :
  forall A (x : E.t A) (c c' : KChain A),
    I_kt c -> E.level A x = 0 -> push_kt4 x c = PushOk c' -> I_kt c'.
Proof.
  intros A x c c' HI Hx Hp.
  destruct (@push_kt4_preserves_colors_leveled A x c c' HI Hx Hp)
    as [Hcc' Hwl'].
...
Proof.
  ...
  - (* KEnding *)
    cbn in Hwl.
    destruct b; intros Hp; try discriminate;
      injection Hp as Hp1 Hp2; subst;
      cbn in Hwl;
      repeat match goal with H : _ /\ _ |- _ => destruct H end;
      (split; [cbn; exact I | cbn; repeat split; auto]).
  - cbn in Hcc. contradiction.
  - pose proof Hwl as Hwl0. cbn in Hwl0. destruct Hwl0 as [Hpkt Hwltail].
    inversion Hpkt as [|? ? ? ? Hpre Hinner Hsuf]; subst.
    destruct col.
    + (* Green: pop then wrap *)
      ...
      destruct pre as [|a|a b1|a b1 c1|a b1 c1 d|];
        ...
        repeat match goal with H : _ /\ _ |- _ => destruct H end.
      * (* B2 -> wrap with B1 *)
        destruct (yellow_wrap_pr (B1 b1) i suf tail) as [|c''] eqn:Hw;
          [discriminate |].
        injection Hp as Hp1 Hp2; subst.
        eapply yellow_wrap_pr_preserves_colors_leveled
          with (k := 0) (pre := B1 b1);
         ...
         | exact Hw ].
      * (* B3 -> wrap with B2 *)
        destruct (yellow_wrap_pr (B2 b1 c1) i suf tail) as [|c''] eqn:Hw;
          [discriminate |].
        injection Hp as Hp1 Hp2; subst.
        eapply yellow_wrap_pr_preserves_colors_leveled
          with (k := 0) (pre := B2 b1 c1);
         [ cbn; exact I
         | apply buf5_green_shape_not_red; exact Hgs
         | exact Hyr
         | exact Htc
    ...
    + (* Yellow *)
      ...
      destruct pre as [|a|a b1|a b1 c1|a b1 c1 d|];
        ...
        repeat match goal with H : _ /\ _ |- _ => destruct H end.
      * (* B1: underflow repair *)
        destruct (green_of_red_k (KCons Red (PNode B0 i suf) tail))
          as [c''|] eqn:Hrep; [| discriminate].
        injection Hp as Hp1 Hp2; subst.
        assert (Hccred :
            colors_consistent (KCons Red (PNode B0 i suf) tail))
          by (cbn; repeat split; assumption).
        assert (Hwlred :
            well_leveled_at 0 (KCons Red (PNode B0 i suf) tail)).
        { cbn. split; [| exact Hwltail].
          constructor; [cbn; exact I | exact Hinner | exact Hsuf]. }
        split.
        -- eapply green_of_red_k_preserves_colors_consistent;
             [exact Hccred | exact Hrep].
        -- eapply green_of_red_k_preserves_well_leveled;
...
Proof.
  ...
  - pose proof Hwl as Hwl0. cbn in Hwl0. destruct Hwl0 as [Hpkt Hwltail].
    inversion Hpkt as [|? ? ? ? Hpre Hinner Hsuf]; subst.
    destruct col.
    + (* Green: eject then wrap (suffix side) *)
      pose proof Hcc as Hcc0. cbn in Hcc0.
      destruct Hcc0 as [[Hgp Hgs] [Hyr [Htc Htail]]].
      destruct suf as [|a|a b1|a b1 c1|a b1 c1 d|];
        cbn in Hgs; try contradiction;
        ...
        repeat match goal with H : _ /\ _ |- _ => destruct H end.
      * (* B2 -> wrap with B1 *)
        destruct (yellow_wrap_pr pre i (B1 a) tail) as [|c''] eqn:Hw;
          [discriminate |].
        injection Hp as Hp1 Hp2; subst.
        eapply yellow_wrap_pr_preserves_colors_leveled
          with (k := 0) (suf := B1 a);
         ...
         | exact Hw ].
      * (* B3 -> wrap with B2 *)
        destruct (yellow_wrap_pr pre i (B2 a b1) tail) as [|c''] eqn:Hw;
          [discriminate |].
        injection Hp as Hp1 Hp2; subst.
        eapply yellow_wrap_pr_preserves_colors_leveled
          with (k := 0) (suf := B2 a b1);
         [ apply buf5_green_shape_not_red; exact Hgp
         | cbn; exact I
         | exact Hyr
         | exact Htc
    ...
    + (* Yellow *)
      ...
      destruct suf as [|a|a b1|a b1 c1|a b1 c1 d|];
        ...
        repeat match goal with H : _ /\ _ |- _ => destruct H end.
      * (* B1: underflow repair *)
        destruct (green_of_red_k (KCons Red (PNode pre i B0) tail))
          as [c''|] eqn:Hrep; [| discriminate].
        injection Hp as Hp1 Hp2; subst.
        assert (Hccred :
            colors_consistent (KCons Red (PNode pre i B0) tail))
          by (cbn; repeat split; assumption).
        assert (Hwlred :
            well_leveled_at 0 (KCons Red (PNode pre i B0) tail)).
        { cbn. split; [| exact Hwltail].
          constructor; [exact Hpre | exact Hinner | cbn; exact I]. }
        split.
        -- eapply green_of_red_k_preserves_colors_consistent;
             [exact Hccred | exact Hrep].
        -- eapply green_of_red_k_preserves_well_leveled;
...
Proof.
  ...
  - exact (@push_kt4_preserves_I_kt A x c c' HI Hx Hc').
